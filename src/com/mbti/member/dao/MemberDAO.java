...
import com.mbti.util.db.MemberSQL;
...
public class MemberDAO {
	...
	public LoginVO login(LoginVO vo) throws Exception{
		...
		try {
			...
			// 3. 4.
			pstmt = con.prepareStatement(MemberSQL.MEMBER_LOGIN);
	...
	public List<MemberVO> list() throws Exception{
		...
		try {
			...
			pstmt = con.prepareStatement(MemberSQL.MEMBER_LIST);
	...
	public int gradeModify(MemberVO vo) throws Exception{
		...
		try {
			...
			System.out.println("MemberDAO.gradeModify().DBSQL.MEMBER_GRADE_MODIFY: "
					+ MemberSQL.MEMBER_GRADE_MODIFY);
			pstmt = con.prepareStatement(MemberSQL.MEMBER_GRADE_MODIFY);
	...
	public MemberVO view(String id) throws Exception{
		...
		try {
			...
			+ MemberSQL.MEMBER_VIEW);
			//3. sql -> DBSQL + 4. 실행객체 + 데이터 셋팅
			pstmt = con.prepareStatement(MemberSQL.MEMBER_VIEW);
			pstmt.setString(1, id); // 가져온 데이터를 담는다.
			// 5. 실행
			rs = pstmt.executeQuery();
			// 5. 실행
			//  - insert, update, delete 쿼리 실행 - int pstmt.executeUpdate()
			...
			// result가 1이면 수정이 한개 수정이 일어 난다.
			// result가 0이면 수정이 되지 않았다.(조건에 맞는 데이터가 없다.)
			System.out.println("MemberDAO.view().result : " + rs);
	...
	public int write(MemberVO vo) throws Exception {
		...
		try {
			...
			con = DBInfo.getConnection();
			// 3. sql + 4. 실행객체
			pstmt = con.prepareStatement(MemberSQL.MEMBER_WRITE);
			pstmt.setString(1, vo.getId());
			pstmt.setString(2, vo.getPw());
	...
	public String checkId(String id) throws Exception {
		...
		try {
			...
			System.out.println("MemberDAO.write().DBSQL.MEMBER_CHECK_ID: "
					+ MemberSQL.MEMBER_CHECK_ID);
			pstmt = con.prepareStatement(MemberSQL.MEMBER_CHECK_ID);
			// sql 쿼리에 포함되어 있는 ? 의미에 맞는 데이터를 순서대로 셋팅
